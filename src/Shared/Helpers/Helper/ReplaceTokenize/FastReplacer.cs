...
using System.Text;
namespace Helpers.Helper.ReplaceTokenize
{
    public class FastReplacer
    {
        ...
        {
            ValidateToken(token, text, false);
            if (OccurrencesOfToken.TryGetValue(token, value: out var occurrences) && occurrences.Count > 0)
            {
                var snippet = new FastReplacerSnippet(text);
                foreach (var occurrence in occurrences)
        ...
        public bool Contains(string token)
        {
            ...
            return OccurrencesOfToken.TryGetValue(token, out var occurrences) && occurrences.Count > 0;
        }
